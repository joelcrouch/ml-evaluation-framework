# scripts/seed_dense_test_cases.py
import os
import json
import requests
import sys
...
GOLDEN_DATASET_PATH = "data/dense_golden_dataset.json"

def main():
    """
    Seeds the database with dense time series test cases from the golden dataset.
    """
    print(f"--- Seeding Dense Test Cases from Golden Dataset ---")
    ...
    if not os.path.exists(GOLDEN_DATASET_PATH):
        ...
        print("  python scripts/train_dense_time_series.py")
        return
    ...
    for case in golden_cases:
        ...
        payload = {
            "test_case_name": f"Dense TS Prediction - Case {case_id}",
            ...
            "tags": ["dense", "relu", "time_series", "temperature", "single_step"],
    ...
    print(f"{'='*60}")
     ...
     #  curl -X 'POST' 'http://localhost:8000/api/v1/runs/' -H 'Content-Type: application/json' -d '{"model_name": "dense_model", "model_version": "1.0", "model_type": "time_series_dense"}'
    if total_created > 0:
        print("\nNext steps:")
        # Fix for the SyntaxError: unterminated string literal
        print("  1. Create a model run:\n     curl -X 'POST' 'http://localhost:8000/api/v1/runs/' \
       -H 'Content-Type: application/json' \
       -d '{\"model_name\": \"dense_model\", \"model_version\": \"1.0\", \"model_type\": \"time_series_dense\"}'")
        print("\n  2. Run evaluation:")

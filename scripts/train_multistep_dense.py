# scripts/train_multistep_dense.py
import os
...
def main(args):
    """Main function to train the Multi-step Dense model and generate datasets."""
    ...
    # --- UPDATED: Create a new window for the multi-step model ---
    CONV_WIDTH = 3
    ...
    # --- UPDATED: Create and compile the Multi-step Dense model ---
    multistep_dense_model = tf.keras.Sequential([
        # Take the input window and flatten it to a single vector
        tf.keras.layers.Flatten(),
        tf.keras.layers.Dense(units=32, activation='relu'),
        tf.keras.layers.Dense(units=32, activation='relu'),
        tf.keras.layers.Dense(units=1),
        # Add back the time dimension for compatibility with our single-step output format
        tf.keras.layers.Reshape([1, -1]),
    ])

    multistep_dense_model.compile(
        loss=tf.keras.losses.MeanSquaredError(),
        metrics=[tf.keras.metrics.MeanAbsoluteError()]
    )

    # --- Train the Multi-step Dense model ---
    MAX_EPOCHS = 20
    ...
    history = multistep_dense_model.fit(conv_window.train, epochs=MAX_EPOCHS,
                                        validation_data=conv_window.val,
                                        callbacks=[early_stopping],
                                        verbose=2)
    print("✅ Multi-step Dense model trained!")

    model_path = os.path.join(MODEL_DIR, "multistep_dense_model.keras")
    print(f"\nSaving Multi-step Dense model to {model_path}...")
    multistep_dense_model.save(model_path)
    print("✅ Multi-step Dense model saved!")
    ...
    create_golden_dataset(
        test_df=test_df,
        window_config={
            'input_width': conv_window.input_width,
            'label_width': conv_window.label_width,
            'shift': conv_window.shift,
            'label_column': 'T (degC)'
        },
        num_samples=args.num_samples,
        output_path='data/multistep_dense_golden_dataset.json'
    )

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train a Multi-step Dense Time Series model.")
    parser.add_argument(
        "--num_samples",
        type=int,
        default=50,

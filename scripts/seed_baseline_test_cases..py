# scripts/seed_baseline_test_cases.py
import os
import json
import requests
import sys
...
GOLDEN_DATASET_PATH = "data/baseline_golden_dataset.json"

def main():
    """
    Seeds the database with baseline time series test cases from the golden dataset.
    """
    print(f"--- Seeding Baseline Test Cases from Golden Dataset ---")
    ...
    if not os.path.exists(GOLDEN_DATASET_PATH):
        ...
        print("  python ml_eval/core/implementations/train_baseline_time_series.py")
        return
    ...
    for case in golden_cases:
        ...
        payload = {
            "test_case_name": f"Baseline TS Prediction - Case {case_id}",
            ...
            "tags": ["baseline", "time_series", "temperature", "single_step"],
    ...
    print(f"  ✅ Successfully created: {total_created}/{len(golden_cases)} test cases")
    if failed > 0:
    ...
    if total_created > 0:
        ...
        print("       -d '{\"model_name\": \"baseline_model\", \"model_version\": \"1.0\", \"model_type\": \"baseline_time_series\"}'")
        print("\n  2. Run evaluation:")
        print("     python scripts/run_evaluation.py <RUN_ID>")
        print("\n  3. Generate report:")
        print("     python scripts/generate_report_time_series.py <RUN_ID>")
